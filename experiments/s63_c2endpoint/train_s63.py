#!/usr/bin/env python
"""S63 CPT: declaration-endpoint ablation of the S60 Chronos-2 retrofit.

The S60 full recipe = mechanism-diverse missingness, half DECLARED (content
zeroed + flag -- the declaration endpoint), half alpha-blend filled. This arm
drops the declaration endpoint: identical to train_s60.py in every other respect
  init     stock models_local/chronos-2 (fp32)
  corpus   s45_pretrain/corpus_cache via t45.Sampler (real:synthetic 85:15, capped)
  regime   "mechdiv_filldiv" (t45.augment, verbatim): 50% of windows clean; the
           rest get mechanism ~ U{mcar, block, mnar_high, mnar_extreme},
           rate ~ U(0.05, 0.7); ALL holed windows alpha-blend filled
           (content = (1-a)*linear + a*truth, a ~ U(0,1), + flag) -- endpoint
           share 0 (mechdiv's `rng.random() < 0.5` declaration branch removed)
  iface    dual (fill content AND flag reach the input patch embedding)
  loss     Chronos-2's NATIVE quantile loss (model._compute_loss)
  budget   5000 steps, bs 1024 (4x256 grad accumulation), AdamW lr 1e-4 wd 0.01,
           warmup 100, linear decay to 0, grad clip 1.0, NaN-gradient guard,
           fp32 with TF32 matmul -- all as train_s60.py

The Chronos-Bolt analogue is ladder arm P5 (blend-only), which collapses 3-19x
vs stock on zero fill. Gate G2' (replica vs stock forward with STOCK weights)
...
REGIME = "mechdiv_filldiv"   # mechdiv with declaration-endpoint share 0
...
def main():
    ...
    tag = "arm_BLEND" + (f"_s{args.seed}" if args.seed != SEED else "") + \
    ...
    for it in range(steps):
        ...
        while len(ctxs) < bs:
            c, f = next(gen)
            c, o = t45.augment(c, REGIME, rng, wi)
            wi += 1
            ctxs.append(c)
            obss.append(o)
            futs.append(f)
    ...
    torch.save(model.state_dict(), os.path.join(CK, f"{tag}.pt"))
    log = {"arm": "BLEND", "iface": "dual", "regime": REGIME, "init": "stock",
           ...
           "ablation": "S60 mechdiv with declaration-endpoint share 0: no holed "
                       "window is ever content-zeroed; all are alpha-blend filled",
