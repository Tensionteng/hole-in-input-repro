...
comparison is paired per dataset per fill. Output: s32_q50.json.
"""
import json
import os
import sys
        ...
        for how in s32.FILLS:
            ...
            try:
                ...
                print(f"[{how} FAIL {type(e).__name__}: {str(e)[:60]}]", end="",
                      flush=True)
            res["cells"][f"q50-base|{dsfreq}|{how}"] = {
                "mase_median": v, "n": int(len(ctx)),
                "ctx_nan_frac": float((~np.isfinite(ctx)).mean())}
            line += f"{how}={v:7.3f} "
        print(line, flush=True)
        json.dump(res, open(os.path.join(HERE, "s32_q50.json"), "w"), indent=1)
    print("\nwrote s32_q50.json")
