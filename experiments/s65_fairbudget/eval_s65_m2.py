#!/usr/bin/env python
"""S65 eval, Moirai 2.0 fair-budget control: stock vs M2CTL vs M2W50 on the
paper's grid -- eval_s61.py's protocol verbatim, only the arm set differs.
...
Arms:
  ...
  M2CTL   S65 no-missingness CPT control (filtered regime; corpus-quality filter
          kept identical) -- the fair-budget analogue of s61's CPT arm
  M2W50   WiSE-FT 0.5*stock + 0.5*M2CTL
...
identical to the s61 run (same code, same n_win, same seeds).
"""
import argparse
import json
import os
...
ARM_FILE = {"M2CTL": "m2_nomiss.pt", "M2W50": "m2_nomiss_w50.pt"}
BATCH = 1024
...
def main():
    ...
    ap.add_argument("--arms", default="stock,M2CTL,M2W50")
    ...
    if "clean" in args.parts or "grid" in args.parts:
        out = os.path.join(HERE, "s65_m2_grid.json")
        ...
        res.setdefault("meta", {"grid": "DS9 x 4 mechs x rates {0.3,0.7} x alphas "
                                ...
                                "control": "M2CTL/M2W50 = s65 no-missingness CPT "
    ...
    if "probe" in args.parts:
        out = os.path.join(HERE, "s65_m2_probe.json")
        res = json.load(open(out)) if os.path.exists(out) else {}
    ...
    if "leaderboard" in args.parts:
        out = os.path.join(HERE, "s65_m2_leaderboard.json")
        res = json.load(open(out)) if os.path.exists(out) else {}
        res.setdefault("meta", {
