#!/usr/bin/env python
"""S60 seed replications: per-seed summaries + the combined s60_seeds.json.
...
eval_s60.py --suffix _s<seed> -- and recomputes the headline metrics with the
EXACT formulas of analyze_s60.py:

  closure@a    (e_stock - e_arm)/(e_stock - 1), declared view, median over
               cells with stock excess e_stock - 1 > 0.02
...
Writes s60_summary_s<seed>.json for every seed (same schema as
s60_summary.json; seed 20260901's file is derived from the original artifacts
and is a NEW file -- nothing existing is overwritten) and the combined
s60_seeds.json with mean +/- sd across the three seeds for closure@1 and
...
def analyze_one(seed):
    """analyze_s60.py's metric set, computed on one seed's JSONs."""
    grid = json.load(open(os.path.join(HERE, f"s60_grid{sfx(seed)}.json")))
    cells = grid["grid"]
    out = {"meta": {"seed": seed,
                    ...
                    "median",
                    "source": f"s60_grid{sfx(seed)}.json / s60_probe"
                    f"{sfx(seed)}.json / s60_leaderboard{sfx(seed)}.json"}}
    ...
    probe = json.load(open(os.path.join(HERE, f"s60_probe{sfx(seed)}.json")))
    ...
    lb = json.load(open(os.path.join(HERE,
                                     f"s60_leaderboard{sfx(seed)}.json")))["grid"]
...
def main():
    ...
    for seed in SEEDS:
        ...
        path = os.path.join(HERE, f"s60_summary_s{seed}.json")
    ...
    combined = {
        "meta": {
            "experiment": "s60_c2retrofit: Chronos-2 120M, 5000-step mechdiv "
                          "CPT from stock + WiSE-FT 0.5",
            "seeds": list(SEEDS),
            ...
            "attack": "bad-fill attack kept single-seed (20260901, "
                      "s60_attack.json) per task scope",
        },
        "seeds": {s: {
    ...
    path = os.path.join(HERE, "s60_seeds.json")
