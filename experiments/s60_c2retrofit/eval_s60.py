#!/usr/bin/env python
"""S60 eval: stock Chronos-2 vs CPT vs c2-retrofit (W50) on the paper's grid.
...
Arms and interfaces:
  ...
  CPT     S60 continued-pretrained checkpoint (mechdiv, dual interface)
  W50     WiSE-FT 0.5*stock + 0.5*CPT -- the "c2-retrofit" (dual interface)
...
ARM_IFACE = {"stock": "native", "CPT": "dual", "W50": "dual"}
...
def main():
    ...
    ap.add_argument("--suffix", default="",
                    help="seed-replication suffix (e.g. _s20260902): load "
                         ...
                         "s60_{grid,probe,leaderboard}{suffix}.json; default "
    ...
    if "clean" in args.parts or "grid" in args.parts:
        out = os.path.join(HERE, f"s60_grid{args.suffix}.json")
    ...
    if "probe" in args.parts:
        out = os.path.join(HERE, f"s60_probe{args.suffix}.json")
        res = json.load(open(out)) if os.path.exists(out) else {}
    ...
    if "leaderboard" in args.parts:
        out = os.path.join(HERE, f"s60_leaderboard{args.suffix}.json")
        res = json.load(open(out)) if os.path.exists(out) else {}
        res.setdefault("meta", {
