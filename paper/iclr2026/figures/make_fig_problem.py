#!/usr/bin/env python
"""Figure 1 (intro): the missingness problem on the field's own data, three panels.

Same measurements as make_fig_charac.py (which this supersedes): mechanism-not-rate,
the silent interval failure, and the fill reversal between two real deployments.
No panel letters -- the caption reads left/middle/right. Writes figures/fig_problem.pdf.
"""
import json, os
import numpy as np
...
# middle: the silent failure, as a trajectory in (width, coverage)
...
# right: the reversal on two real deployments
ax = axes[2]
...
out = os.path.join(HERE, "fig_problem.pdf")
